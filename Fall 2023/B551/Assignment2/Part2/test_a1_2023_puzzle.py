...
def check_puzzle(mapX, path_length):
    ...
    assert set(path_found).issubset(
        valid_moves_set
    ), f"Invalid moves: {list(set(path_found) - valid_moves_set)}"
    ...
    assert cstate == path_finder(
        mapX, path_found
    ), "Canonical state not reached. The moves do not solve the puzze!"
